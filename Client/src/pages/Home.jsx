...
export const Home = () => {
  ...
  const [searchId, setsearchId] = useState('')
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://127.0.0.1:6001/api/${searchid}/get`, options);
  ...
  <div className="container px-5 py-24 mx-auto">
    ...
    <div className="flex lg:w-2/3 w-full sm:flex-row flex-col mx-auto px-8 sm:space-x-4 sm:space-y-0 space-y-4 sm:px-0 items-end">
      ...
      {/* <div className="relative flex-grow w-full">
        <label for="email" className="leading-7 text-sm text-gray-600">Enter</label>
        <input type="email" id="Reason" name="email" className="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-transparent focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"/>
      </div> */}
